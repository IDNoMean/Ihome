...
class PicCodeHandler(BaseHandlers):
    ...
    def get(self):
        ...
        try:
            ...
            self.redis.setex("pre_code_%s" % cur_code_id, PIC_CODE_EXPIRES_SECONDS, text)
...
class SMSCodeHandler(BaseHandlers):
    ...
    def post(self):
        ...
        try:
            real_piccode = self.redis.get("pic_code_%s" %piccode)
        except Exception as e:
            logging.error(e)
        ...
        sql = "select counts(*) counts from ih_user_profile where up_mobile=%s"
