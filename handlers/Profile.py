...
class AvatarHandler(BaseHandlers):
...
class ProfileHandler(BaseHandlers):
    ...
    def get(self):
        ...
        self.write({"errcode": RET.OK, "errmsg": "OK","data": {"user_id": user_id, "name": ret["up_name"], "mobile": ret["up_mobile"],
                             "avatar": img_url}})

class NameHandler(BaseHandlers):
...
class AuthHandler(BaseHandlers):
    ...
    def get(self):
        ...
        try:
            ret = self.db.execute_rowcount("select up_real_name,up_id_card from ih_user_profile where up_user_id=%s", user_id)
        except Exception as e:
            logging.error(e)
